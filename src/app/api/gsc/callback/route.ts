...
export async function GET(request: Request) {
  try {
    ...
    db.update(tenants)
      .set({
        gscRefreshToken: setTenantSecret(tokens.refresh_token),
        gscAccessToken: setTenantSecret(tokens.access_token),
        gscSiteUrl: siteUrl,
        gscConnected: 1,
      })
...
<p>${siteUrl ? `Sitio: ${escapeHtml(siteUrl)}` : "Puedes configurar el sitio manualmente"}</p>
...
<body><h1>Error de conexión</h1><p>${escapeHtml(message)}</p></body></html>`,
